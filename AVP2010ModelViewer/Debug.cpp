...
int bbox_to_vertex(std::vector <bbox> &bboxes, DWORD* g_points)
{
	...
	*g_points = size_of_vertex_data;
	...
	for (std::vector<bbox>::iterator it = bboxes.begin(); it != bboxes.end(); ++it)
	{
		...
		for (int i = 0; i < 24; i++)
		{
			XMVECTOR new_p = XMVector3Transform(XMLoadFloat3(&(vertices[i + r].Pos)) - p, Q) + p;
			XMStoreFloat3(&(vertices[i + r].Pos), new_p);
		}
		r += 24;
	}
	...
	hr = g_pd3dDevice->CreateBuffer(&bd, &InitData, &g_pVertexBuffer_generic_bbox);
	if (FAILED(hr))
	{
...
int bbox_to_vertex_to_buffer(std::vector <bbox> &bboxes, DWORD* g_points, ID3D11Buffer** buffer)
{
	...
	*g_points = size_of_vertex_data;
	...
	D3D11_SUBRESOURCE_DATA InitData = {};
	InitData.pSysMem = vertices;
	hr = g_pd3dDevice->CreateBuffer(&bd, &InitData, buffer);
	if (FAILED(hr))
	{
		dbgprint("Helpers", "Bbox CreateBuffer failed ! \n");
		return 0;
...
void Dump_floats(const char* name, void* data, unsigned int count)
{
	...
	{
		int lines = count / 16;
		int rows = (count - lines * 16) / 4;
		for (int i = 0; i < lines; i++)
		{
			dbgprint(name, "%f %f %f %f \n", ((float*)data)[i * 4],
				((float*)data)[i * 4 + 1], ((float*)data)[i * 4 + 2], \
				((float*)data)[i * 4 + 3]);
		}
		for (int i = 0; i < rows; i++)
		{
			dbgprint(name, "%f \n", ((float*)data)[lines * 4 + i]);
		}
		if (count - lines * 16 - rows * 4 > 0)
		{
			memset(g_dump_hex_line, 0, 81);
			for (unsigned int i = 0; i < count - lines * 16; i++)
			{
